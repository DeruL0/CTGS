...
def add_ct_optimization_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument("--ct_value_lr", type=float, default=1e-3)
    parser.add_argument("--ct_attenuation_lr", type=float, default=1e-3)
...
def extract_ct_optimization_args(args) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        ct_value_lr=float(getattr(args, "ct_value_lr", 1e-3)),
        ct_attenuation_lr=float(getattr(args, "ct_attenuation_lr", getattr(args, "ct_value_lr", 1e-3))),
    )
