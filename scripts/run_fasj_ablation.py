"""Run the focused Feature-Adaptive Stratified Jittered bulk-init ablation."""
...
BASE_FLAGS = [
    ...
    "100",
    "--ct_gap_aware_reseed",
    "--no-ct_material_coverage_completion",
...
VARIANTS = {
    "A0_iso_spacing2": {
        ...
        "flags": [
            ...
            "1.3",
            "--no-ct_bulk_lattice_anisotropic",
        ],
    },
    "A1_iso_spacing3": {
        "label": "iso spacing=3 sigma=2.0",
        "flags": [
            "--ct_bulk_init_mode",
            "contained_lattice",
            "--ct_bulk_lattice_spacing_vox",
    ...
    "A2_fasj_no_jitter": {
        "label": "adaptive spacing without jitter",
        "flags": [
            "--ct_bulk_init_mode",
            "feature_adaptive",
            "--no-ct_feature_adaptive_jitter",
    ...
    "A3_fasj_jitter": {
        "label": "adaptive spacing + stratified jitter",
        "flags": [
            "--ct_bulk_init_mode",
            "feature_adaptive",
    ...
    "A4_fasj_jitter_aniso": {
        "label": "adaptive spacing + jitter + anisotropic risky bulk",
        "flags": [
            "--ct_bulk_init_mode",
            "feature_adaptive",
            "--ct_feature_adaptive_jitter",
...
def write_summary(output_root: Path, rows: list[dict[str, str]]) -> None:
    ...
    (output_root / "fasj_summary.tsv").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = (
        ...
        if args.output_root
        else REPO_ROOT / "outputs" / "bunny_smoke" / f"fasj_ablation_{timestamp}"
    )
    ...
    write_summary(output_root, rows)
    print(f"\nWrote summary: {output_root / 'fasj_summary.tsv'}", flush=True)
    if failures:
