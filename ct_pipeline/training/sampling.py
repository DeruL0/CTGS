...
def _sample_far_phase_points(
    ...
    boundary_band_distance: float,
    support_sample_ratio: float = 0.5,
    exterior_air_sample_ratio: float = 0.5,
    device="cuda",
):
    sample_count = int(sample_count)
    if sample_count <= 0:
    ...
    support_count = int(round(float(sample_count) * support_ratio))
    support_count = max(0, min(sample_count, support_count))
    air_count = sample_count - support_count
    support_candidates = field_pools.get("material_deep_pool")
    if _candidate_count(support_candidates) == 0:
        support_candidates = _cached_or_filter_candidates(field_pools, "support", signed_distance_field, boundary_band_distance, keep_boundary=False)
    void_air_candidates = field_pools.get("void_air_pool", field_pools.get("void_air"))
    exterior_air_candidates = field_pools.get("exterior_air_pool")
    if _candidate_count(exterior_air_candidates) == 0:
        exterior_air_candidates = _cached_or_filter_candidates(field_pools, "exterior_air", signed_distance_field, boundary_band_distance, keep_boundary=False)
    if _candidate_count(exterior_air_candidates) == 0:
        exterior_air_candidates = _cached_or_filter_candidates(field_pools, "air", signed_distance_field, boundary_band_distance, keep_boundary=False)

    points = []
    targets = []
    support_points = _sample_occupancy_points(support_candidates, support_count, spacing_zyx, device=device)
    if support_points.numel() > 0:
        points.append(support_points)
        targets.append(torch.ones((support_points.shape[0],), dtype=torch.float32, device=device))

    if air_count > 0:
        void_available = _candidate_count(void_air_candidates)
        exterior_available = _candidate_count(exterior_air_candidates)
        exterior_ratio = min(max(float(exterior_air_sample_ratio), 0.0), 1.0)
        if void_available <= 0 and exterior_available > 0:
            exterior_count = int(air_count)
            void_count = 0
        elif exterior_available <= 0 and void_available > 0:
            exterior_count = 0
            void_count = int(air_count)
        elif void_available > 0 and exterior_available > 0:
            exterior_count = min(int(round(float(air_count) * exterior_ratio)), int(air_count) - 1)
            exterior_count = max(0, exterior_count)
            void_count = int(air_count) - exterior_count
        else:
            exterior_count = 0
            void_count = 0

        air_sources = (
            (void_air_candidates, void_count),
            (exterior_air_candidates, exterior_count),
        )
        for candidates, source_count in air_sources:
            if source_count <= 0 or _candidate_count(candidates) <= 0:
                continue
            air_points = _sample_occupancy_points(candidates, source_count, spacing_zyx, device=device)
            if air_points.numel() > 0:
                points.append(air_points)
                targets.append(torch.zeros((air_points.shape[0],), dtype=torch.float32, device=device))

    if not points:
        return _ct_empty_points(device=device), torch.empty((0,), dtype=torch.float32, device=device)

    sampled_points = torch.cat(points, dim=0)
    sampled_targets = torch.cat(targets, dim=0)
    sampled_points = _jitter_and_clamp_volume_points(sampled_points, spacing_zyx, volume_shape_dhw, float(jitter_voxels))
    return sampled_points[:sample_count], sampled_targets[:sample_count]
