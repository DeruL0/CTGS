...
__global__ void render_slice_patch_forward_kernel(
    const float* means,
...
__global__ void render_slice_patch_backward_kernel(
    const float* means,
    const float* rotations,
    const float* scales,
    const float* opacity,
...
at::Tensor render_slice_patch_forward_cuda(
    at::Tensor means,
    at::Tensor rotations,
    at::Tensor scales,
    at::Tensor opacity,
    ...
    auto output = at::zeros({patch_h, patch_w}, means.options());
    ...
    render_slice_patch_forward_kernel<<<blocks, kThreads, 0, stream>>>(
    ...
    TORCH_CHECK(error == cudaSuccess, "render_slice_patch_forward kernel launch failed: ", cudaGetErrorString(error));
    return output;
}

std::vector<at::Tensor> render_slice_patch_backward_cuda(
    at::Tensor means,
    at::Tensor rotations,
    at::Tensor scales,
    at::Tensor opacity,
    ...
    TORCH_CHECK(grad_output.dim() == 2, "grad_output must have shape (patch_h, patch_w).");
    TORCH_CHECK(grad_output.size(0) == patch_h && grad_output.size(1) == patch_w, "grad_output shape must match patch size.");
    ...
    render_slice_patch_backward_kernel<<<blocks, kThreads, 0, stream>>>(
        means.contiguous().data_ptr<float>(),
        rotations.contiguous().data_ptr<float>(),
        scales.contiguous().data_ptr<float>(),
        opacity.contiguous().data_ptr<float>(),
    ...
    auto error = cudaGetLastError();
    TORCH_CHECK(error == cudaSuccess, "render_slice_patch_backward kernel launch failed: ", cudaGetErrorString(error));
    return {grad_means, grad_rotations, grad_scales, grad_opacity};
}
