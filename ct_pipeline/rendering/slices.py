...
_WORLD_AXIS_RENDER_CONFIG = {
    ...
    1: {"plane_axis": 1, "dim_h": 2, "dim_w": 0},
    2: {"plane_axis": 2, "dim_h": 1, "dim_w": 0},
}
_DEFAULT_MAX_GAUSSIAN_PIXEL_PAIRS = 4_194_304
...
def _normalize_world_axis(axis) -> int:
    if axis not in _WORLD_AXIS_MAP:
        raise ValueError("world axis must be one of {'x', 'y', 'z'}.")
    return _WORLD_AXIS_MAP[axis]
...
def _build_query_points_from_base(
    ...
    # Slice-index rendering follows the same voxel-center convention as CT
    # preprocessing and training samples.
    rr = rr + float(origin_h) + 0.5
    cc = cc + float(origin_w) + 0.5

    if axis_index == 0:
        x = cc * spacing_x
        y = rr * spacing_y
        z = torch.full_like(rr, (float(slice_idx) + 0.5) * spacing_z)
    elif axis_index == 1:
        x = cc * spacing_x
        y = torch.full_like(rr, (float(slice_idx) + 0.5) * spacing_y)
        z = rr * spacing_z
    else:
        x = torch.full_like(rr, (float(slice_idx) + 0.5) * spacing_x)
        y = cc * spacing_y
        z = rr * spacing_z
...
def _grid_axis_to_world(values: torch.Tensor, lower: float, upper: float, count: int) -> torch.Tensor:
    if count <= 1:
        midpoint = 0.5 * (float(lower) + float(upper))
        return torch.full_like(values, midpoint)
    return float(lower) + (float(upper) - float(lower)) * (values / float(count - 1))


def _build_world_query_points_from_base(
    rr: torch.Tensor,
    cc: torch.Tensor,
    axis_index: int,
    slice_coord: float,
    ...
    patch_height, patch_width = [int(value) for value in patch_size_hw]
    (lower_h, upper_h), (lower_w, upper_w) = bounds_hw

    world_h = _grid_axis_to_world(rr, lower_h, upper_h, patch_height)
    world_w = _grid_axis_to_world(cc, lower_w, upper_w, patch_width)

    if axis_index == 0:
        x = torch.full_like(world_h, float(slice_coord))
        y = world_w
        z = world_h
    elif axis_index == 1:
        x = world_w
        y = torch.full_like(world_h, float(slice_coord))
        z = world_h
    else:
        x = world_w
        y = world_h
        z = torch.full_like(world_h, float(slice_coord))

    return torch.stack((x, y, z), dim=-1).reshape(-1, 3)


def _adaptive_gaussian_chunk_size(query_count: int, requested_chunk_size: int, max_pair_count: int) -> int:
    query_count = max(1, int(query_count))
    requested_chunk_size = max(1, int(requested_chunk_size))
    max_pair_count = max(1, int(max_pair_count))
    budgeted_chunk_size = max(1, max_pair_count // query_count)
    return min(requested_chunk_size, budgeted_chunk_size)
...
def render_ct_slice_world_patch(
    ...
    patch_size_hw,
    gaussians_per_chunk: int = 2048,
    max_gaussian_pixel_pairs: int = _DEFAULT_MAX_GAUSSIAN_PIXEL_PAIRS,
    patch_grid_cache: Optional[CTPatchGridCache] = None,
    render_impl: Optional[Callable[..., torch.Tensor]] = None,
    output_mode: str = "occupancy",
):
    axis_index = _normalize_world_axis(axis)
    if isinstance(patch_size_hw, int):
        patch_size_hw = (int(patch_size_hw), int(patch_size_hw))
    else:
        patch_size_hw = (int(patch_size_hw[0]), int(patch_size_hw[1]))
    patch_height, patch_width = patch_size_hw
    if patch_height < 1 or patch_width < 1:
        raise ValueError("patch_size_hw must be >= 1 along both dimensions.")
    ...
    patch_grid_cache = patch_grid_cache if patch_grid_cache is not None else CTPatchGridCache()
    rr, cc = patch_grid_cache.get(patch_size_hw, render_state.device, render_state.dtype)
    query_points = _build_world_query_points_from_base(rr, cc, axis_index, float(slice_coord), bounds_hw, patch_size_hw)
    axis_cfg = _WORLD_AXIS_RENDER_CONFIG[axis_index]
    chunk_size = _adaptive_gaussian_chunk_size(
        query_points.shape[0],
        int(gaussians_per_chunk),
        int(max_gaussian_pixel_pairs),
    )

    render_impl = render_impl or _render_ct_slice_patch_impl
    density_values = render_impl(
        query_points,
        torch.as_tensor(float(slice_coord), device=render_state.device, dtype=render_state.dtype),
        render_state.means,
        render_state.rotations,
        render_state.scales,
        render_state.opacity,
    ...
    if output_mode == "density":
        return density_values.clamp_min_(0.0)
