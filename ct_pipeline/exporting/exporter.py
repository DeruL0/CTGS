...
class CTExporter:
    ...
    def export_metrology_mesh(self, model, path, resolution=0.05, method="sugar", **mesh_kwargs):
        mesh = meshing_ct(None, model, resolution=resolution, threshold=0.5, method=method, **mesh_kwargs)
        export_path = Path(path)
        if export_path.suffix == "":
            export_path = export_path.with_suffix(".ply")
