...
def _load_raw(path: Path, sidecar: Optional[Dict[str, Any]]) -> Tuple[np.ndarray, Dict[str, Any]]:
    if sidecar is None:
...
def _load_tiff(path: Path, sidecar: Optional[Dict[str, Any]]) -> Tuple[np.ndarray, Dict[str, Any]]:
    if tifffile is None:
    ...
    embedded_spacing = _read_tiff_spacing(metadata_file)
    if sidecar is not None and "spacing" in sidecar:
        spacing_zyx = _as_float_list(sidecar["spacing"], 3, "spacing")
    elif embedded_spacing is not None:
        spacing_zyx = embedded_spacing
    else:
        raise ValueError("TIFF spacing was not found in embedded metadata. Provide a JSON sidecar with spacing.")
...
def _read_tiff_spacing(path: Path) -> Optional[List[float]]:
    if tifffile is None:
        return None

    with tifffile.TiffFile(str(path)) as tif:
        ome_spacing = _read_ome_spacing(tif)
        if ome_spacing is not None:
            return ome_spacing

        imagej_spacing = _read_imagej_spacing(tif)
...
def _read_ome_spacing(tif: Any) -> Optional[List[float]]:
...
def _read_imagej_spacing(tif: Any) -> Optional[List[float]]:
    ...
    unit_scale = _resolution_unit_scale_to_mm(resolution_unit_tag.value if resolution_unit_tag is not None else None)
...
def _load_dicom(path: Path) -> Tuple[np.ndarray, Dict[str, Any]]:
    if pydicom is None:
        raise ImportError("pydicom is required to load DICOM CT volumes.")

    headers = _collect_dicom_headers(path)
    if not headers:
        raise ValueError("No readable DICOM slices were found at: {0}".format(path))

    reference_header = headers[0][1]
    x_dir, y_dir, z_dir = _extract_dicom_orientation(reference_header)
    sorted_headers = _sort_dicom_headers(headers, z_dir)
    ...
    spacing_zyx = _extract_dicom_spacing(sorted_headers, z_dir)
...
def _collect_dicom_headers(path: Path) -> List[Tuple[Path, Any]]:
    if pydicom is None:
        raise ImportError("pydicom is required to load DICOM CT volumes.")
    ...
    if path.is_dir():
        series_ids = {
            ...
            if getattr(header, "SeriesInstanceUID", None) is not None
        }
        if len(series_ids) > 1:
            raise ValueError("Multiple DICOM series were found in {0}. Pass a representative slice file instead.".format(path))

    return headers


def _extract_dicom_orientation(header: Any) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def _sort_dicom_headers(headers: List[Tuple[Path, Any]], z_dir: np.ndarray) -> List[Tuple[Path, Any]]:
...
def _extract_dicom_spacing(headers: List[Tuple[Path, Any]], z_dir: np.ndarray) -> List[float]:
...
class CTVolumeLoader:
    ...
    def load(self, path: str, fmt: str = "auto", raw_meta_path: Optional[str] = None) -> np.ndarray:
        ...
        resolved_format = _resolve_format(input_path, fmt)
        sidecar = None
        if raw_meta_path is not None:
            sidecar = _read_json(Path(raw_meta_path).expanduser().resolve())

        if resolved_format == "raw":
            volume, metadata = _load_raw(input_path, sidecar)
        elif resolved_format == "tiff":
            volume, metadata = _load_tiff(input_path, sidecar)
        elif resolved_format == "dicom":
            volume, metadata = _load_dicom(input_path)
        else:
